import { Church, HeartPulse, Landmark, Sparkles, Store, Trees, Users } from "lucide-react";
...
const PLACES = [
  ...
  },
  {
    id: "church" as const,
    title: "Церковь",
    desc: "Жрецы Первого Света исцелят раны мгновенно — за щедрое пожертвование.",
    img: null,
    icon: Church,
    tag: "исцеление",
  },
...
export function CityView() {
  ...
  const hpPct = Math.floor((hp / stats.hp) * 100);
  const healthy = hp >= stats.hp - 0.5;
  const restLeft = Math.ceil((stats.hp - hp) / (stats.hp / REGEN_PER_SEC_DIV));
      ...
      <div className="relative z-10 flex min-h-[calc(100vh-120px)] flex-col justify-between p-5 sm:p-8">
        <div className="anim-fade-up max-w-2xl pt-4 sm:pt-8">
          ...
          <p className="mt-3 max-w-lg text-sm leading-relaxed text-white/60">
            Здесь раны затягиваются сами — по 1/{REGEN_PER_SEC_DIV} запаса жизни в секунду.
            Спешите? Выпейте зелье или закажите чудо в церкви.
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-black/40 px-3 py-1 text-[11px] text-white/65 backdrop-blur">
              <HeartPulse className="h-3.5 w-3.5 text-blood-400" />
              {healthy ? (
                "Герой полон сил"
              ) : (
                <>
                  Восстановление: {hpPct}%
                  <span className="font-mono2 text-gold-300">
                    {" "}
                    ещё {Math.floor(restLeft / 60)}:{String(restLeft % 60).padStart(2, "0")}
                  </span>
                </>
              )}
            </span>
        ...
        <div className="grid gap-3 pb-2 sm:grid-cols-2 sm:gap-4 xl:grid-cols-4">
          {PLACES.map((p, i) => (
            <button
              key={p.id}
